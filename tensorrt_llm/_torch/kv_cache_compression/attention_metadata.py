...
def requires_paged_draft_kv_length_domain(
    ...
    """Return whether a separate draft KVCM feeds standard paged attention."""
...
class KVCacheCompressionAwareTrtllmAttentionMetadata(TrtllmAttentionMetadata):
    ...
    Compression can shorten only the target cache while one-model speculative
    decoding keeps a separate draft cache dense. The attention kernel consumes
    both host and device length metadata, and CUDA Graph replay requires stable
    addresses. This adapter materializes both domains and selects the domain
    matching the KV cache manager already chosen by the speculative runtime.
    ...
    def _post_init_with_buffers(self, buffers) -> None:
        ...
        self.draft_kv_length_delta_cpu = torch.empty_like(
            self.kv_lens, device="cpu", pin_memory=prefer_pinned()
        )
