...
class RocketKVCacheManagerV2(KVCacheManagerV2):
    ...
    def fill_kt_block_offsets(self, request_ids: List[int], out: torch.Tensor) -> torch.Tensor:
        ...
        if os.environ.get("KT_LOOP") == "1":
            ...
            return out
        # --- vectorized (perf #1): gather raw page indices (pybind only, no per-req
        # torch dispatch), then do ALL the where/scale/scatter ONCE. Removes the
        # O(BS) torch-dispatch + O(BS) tiny H2D of the loop above. Byte-identical:
        # within each row's valid prefix [0,n_i) BAD->0 else idx*scale; the tail
        # [n_i:] and absent rows are left untouched (mask keeps the current value).
        W = int(out.shape[1])
        rows = []
        idx_lists = []
        for i, req_id in enumerate(request_ids):
