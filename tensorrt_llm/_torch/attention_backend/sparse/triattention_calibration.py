...
Output schema (``torch.save`` dict, consumed by ``TriAttention._load_calibration``):

  E_q       [L, H, D/2] complex   per-(layer, head, freq) Q center
  E_q_norm  [L, H, D/2] float     E[||q_f||]
  R         [L, H, D/2] float     mean resultant length, |E_q| / E_q_norm
  omega     [D/2]       float     RoPE freqs (model inv_freq)
