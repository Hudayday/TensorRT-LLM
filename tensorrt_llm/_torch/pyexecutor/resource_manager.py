...
class BaseKVCacheCompressionManager(BaseResourceManager):
    ...
    def __init__(
        ...
        self.spec_config = spec_config
        self.validate_spec_draft_kv(spec_config)
        ...
        if draft_kv_cache_manager is not None:
            ...
            draft_kv_cache_manager.generation_capacity_only = (
                self.adjusts_generation_kv_length)

    def validate_spec_draft_kv(
            self, spec_config: Optional["DecodingBaseConfig"]) -> None:
        """Reject speculative modes this compression method cannot run with.

        Default: accept everything. Methods that do not touch the draft KV
        (e.g. offloading) work with any speculative mode; methods that evict
        override this with their own acceptance rules.
        """
