...
MemoryOwner makeMemoryOwner(size_t size, StagingBufferMemory memory)
{
    if (size == 0)
    {
        throw std::invalid_argument("StagingBufferManager size must be non-zero");
    }
    switch (memory)
    {
...
void DeviceDeleter::operator()(std::byte* ptr) const noexcept
{
    if (ptr != nullptr)
    {
        TLLM_CU_CHECK_FREE_RESOURCE(cuMemFree(reinterpret_cast<CUdeviceptr>(ptr)));
    }
}
...
StagingBuffer::StagingBuffer(StagingBufferManager& manager, size_t minSize, size_t maxSize, size_t sizeGranularity,
    ...
    , mStream(stream)
{
    mRange = mManager.reserve(minSize, maxSize, sizeGranularity, alignment, stream);
    mSize = mRange->end - mRange->begin;
    mAddress = mManager.baseAddress() + mRange->begin;
}
...
StagingBufferManager::~StagingBufferManager()
{
    ...
    {
        TLLM_CHECK_DEBUG_WITH_INFO(range.retired, "Destroying a staging manager with a live buffer");
    ...
    size_t minSize, size_t maxSize, size_t sizeGranularity, size_t alignment, std::optional<CUstream> stream)
{
    ...
    auto findRange = [this](size_t offset, auto range)
    {
        while (range != mRanges.end() && range->end <= offset)
        {
            ++range;
        }
        while (range != mRanges.begin() && range->begin > offset)
        {
            --range;
        }
        TLLM_CHECK_DEBUG(range != mRanges.end());
        TLLM_CHECK_DEBUG(range->begin <= offset && offset < range->end);
        return range;
    };

    auto splitRange = [this, &findRange](auto hint, size_t offset)
    {
        if (offset == 0)
        {
    ...
    auto reserveFrom = [&](auto scanBegin) -> StagingBufferRange*
    {
        auto runBegin = scanBegin;
        while (runBegin != mRanges.end())
        {
            ...
            while (runEnd != mRanges.end() && runEnd->retired)
            {
            ...
            if (dataBegin <= availableEnd && minSize <= availableEnd - dataBegin)
            {
                ...
                if (stream.has_value())
                {
                    std::vector<CachedCudaEvent const*> constEvents(readyEvents.begin(), readyEvents.end());
                    streamWaitEvents(reinterpret_cast<CudaStream>(*stream), constEvents);
                }
                else
                {
                    synchronizeAll(readyEvents);
