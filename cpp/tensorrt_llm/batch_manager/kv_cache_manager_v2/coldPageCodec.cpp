...
private:
    ...
    {
        TLLM_CHECK(gpuDescs != nullptr && numGpuDescs.value() > 0);
        TLLM_CHECK(mGroups.empty() && mLifeCycleToGroup.empty());
        ...
        for (PoolGroupIndex poolGroupIndex{0}; poolGroupIndex < numGpuDescs; ++poolGroupIndex)
        {
            ...
            TLLM_CHECK(gpuDesc.poolGroupIndex == poolGroupIndex);
            TLLM_CHECK(!gpuDesc.pools.empty());
            TLLM_CHECK(!gpuDesc.slotDesc.variants.empty());
            for (auto const& variant : gpuDesc.slotDesc.variants)
            {
                TLLM_CHECK(variant.lifeCycleId.value() >= 0);
                TLLM_CHECK(variant.lifeCycleId.value() < std::numeric_limits<LifeCycleId::ValueType>::max());
                TLLM_CHECK(variant.coalescedBuffers.stdSize() == gpuDesc.pools.stdSize());
        ...
        for (PoolGroupIndex poolGroupIndex{0}; poolGroupIndex < numGpuDescs; ++poolGroupIndex)
        {
            ...
            for (PoolIndex poolIndex{0}; poolIndex < gpuDesc.pools.size(); ++poolIndex)
            {
                ...
                TLLM_CHECK(pool.poolIndex == poolIndex);
                TLLM_CHECK(pool.baseAddress != 0);
                TLLM_CHECK(pool.slotBytes > 0);
                TLLM_CHECK(pool.slotBytes <= std::numeric_limits<size_t>::max() - coldOffset);
                copyPlans.push_back(
    ...
    bool validate(GroupConfig const* group, void* dstBasePtr, void const* srcBasePtr, PageIndexPair const* pageIndices,
        size_t numBasePages) const
    {
        if (numBasePages == 0)
        {
    ...
    {
        if (!validate<Encode>(group, dstBasePtr, srcBasePtr, pageIndices, numBasePages))
        {
            return false;
        }
        if (numBasePages == 0)
        ...
        if (stream == nullptr || numBasePages > std::numeric_limits<size_t>::max() / group->copyPlans.stdSize())
        {
            return false;
        }
