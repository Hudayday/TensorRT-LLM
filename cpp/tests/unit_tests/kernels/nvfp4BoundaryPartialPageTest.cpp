...
public:
    ...
    {
        if (offset + bytes.size() > mBytes)
        {
            throw std::invalid_argument("DeviceBuffer source exceeds the allocation");
        }
        checkCuda(cudaMemcpy(static_cast<std::uint8_t*>(mPointer) + offset, bytes.data(), bytes.size(),
    ...
    {
        EXPECT_LE(offset + bytes, mBytes);
        std::vector<std::uint8_t> result(bytes);
        EXPECT_EQ(cudaMemcpy(result.data(), static_cast<std::uint8_t const*>(mPointer) + offset, bytes,
...
std::vector<std::uint8_t> makeRawPage(RuntimeKind kind, std::int32_t validTokens, bool zeroTail, std::uint32_t role)
{
    ...
    for (std::int32_t head = 0; head < kNumHeads; ++head)
    {
        for (std::int32_t token = 0; token < kTokensPerPage; ++token)
        {
            ...
            {
                std::size_t const index = (static_cast<std::size_t>(head) * kTokensPerPage + token) * kHeadDim + dim;
                float value = 0.0F;
...
void expectSameValidPrefix(std::vector<std::uint8_t> const& lhs, std::vector<std::uint8_t> const& rhs, RuntimeKind kind,
    std::int32_t validTokens)
{
    ...
    for (std::int32_t head = 0; head < kNumHeads; ++head)
    {
        ...
        {
            std::size_t const offset = (static_cast<std::size_t>(head) * kTokensPerPage + token) * rowBytes;
...
void runPartialPageBatch(RuntimeKind kind)
{
    ...
    pages.reserve(2U * kValidTokenCounts.size());
    for (std::int32_t validTokens : kValidTokenCounts)
    {
        pages.push_back({validTokens, true});
        pages.push_back({validTokens, false});
    }

    std::size_t const rawBytes = rawPageBytes(kind);
    std::size_t const compactBytes = compactPageBytes();
    DeviceBuffer rawInputK(pages.size() * rawBytes);
    DeviceBuffer rawInputV(pages.size() * rawBytes);
    DeviceBuffer rawOutputK(pages.size() * rawBytes);
    DeviceBuffer rawOutputV(pages.size() * rawBytes);
    MappedHostBuffer compact(pages.size() * compactBytes);
    std::vector<Nvfp4BoundaryOffloadPageTask> offloadTasks;
    for (std::size_t page = 0; page < pages.size(); ++page)
    {
        rawInputK.copyFrom(page * rawBytes, makeRawPage(kind, pages[page].validTokens, pages[page].zeroTail, 0));
        rawInputV.copyFrom(page * rawBytes, makeRawPage(kind, pages[page].validTokens, pages[page].zeroTail, 1));
        offloadTasks.push_back({static_cast<std::int32_t>(page), static_cast<std::int32_t>(page)});
    }
    ...
    Nvfp4BoundaryLayerPlan const inputLayer{reinterpret_cast<std::uintptr_t>(rawInputK.data()),
        reinterpret_cast<std::uintptr_t>(rawInputV.data()), rawBytes, rawBytes, 0U, params};
    auto const inputPlan
        = tensorrt_llm::kernels::prepareNvfp4BoundaryPlan({inputLayer}, compactBytes, runtimeType(kind));
    tensorrt_llm::kernels::invokeNvfp4BoundaryOffloadCompress(offloadTasks, inputPlan, compact.deviceBytes(), stream);

    std::vector<Nvfp4BoundaryOnboardPageTask> onboardTasks;
    onboardTasks.reserve(pages.size());
    for (std::size_t page = 0; page < pages.size(); ++page)
    {
        onboardTasks.push_back({static_cast<std::int32_t>(page), static_cast<std::int32_t>(page)});
    }
    Nvfp4BoundaryLayerPlan const outputLayer{reinterpret_cast<std::uintptr_t>(rawOutputK.data()),
        reinterpret_cast<std::uintptr_t>(rawOutputV.data()), rawBytes, rawBytes, 0U, params};
    auto const outputPlan
        = tensorrt_llm::kernels::prepareNvfp4BoundaryPlan({outputLayer}, compactBytes, runtimeType(kind));
    tensorrt_llm::kernels::invokeNvfp4BoundaryOnboardDecompress(
        onboardTasks, outputPlan, compact.deviceBytes(), stream);
    ...
    for (std::size_t pair = 0; pair < kValidTokenCounts.size(); ++pair)
    {
        ...
        {
            auto const& output = key ? rawOutputK : rawOutputV;
            auto const zeroOutput = output.copyToHost(2U * pair * rawBytes, rawBytes);
            auto const staleOutput = output.copyToHost((2U * pair + 1U) * rawBytes, rawBytes);
