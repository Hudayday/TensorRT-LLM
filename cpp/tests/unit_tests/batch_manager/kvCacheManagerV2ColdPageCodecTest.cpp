...
TEST(KvCacheManagerV2ColdPageCodecTest, ConfiguresAllPoolGroupsAtomically)
{
...
TEST(KvCacheManagerV2ColdPageCodecTest, ConcatenatesAndRestoresLargeNonContiguousBatch)
{
    ASSERT_EQ(cudaSetDevice(0), cudaSuccess);
...
TEST(KvCacheManagerV2ColdPageCodecTest, RoundTripsBatchedCopies)
{
    ASSERT_EQ(cudaSetDevice(0), cudaSuccess);
...
TEST(KvCacheManagerV2ColdPageCodecTest, AcceptsUnalignedPoolLayouts)
{
    ASSERT_EQ(cudaSetDevice(0), cudaSuccess);
...
TEST(KvCacheManagerV2ColdPageCodecTest, ValidatesHostIndexArgumentsBeforeSubmission)
{
    ASSERT_EQ(cudaSetDevice(0), cudaSuccess);
