...
Today the framework integrates with KVCacheManagerV2, the KV cache manager built around a flexible, hierarchical storage model. V2 can give different layers pools of different types and sizes, groups layers by their lifecycle, and coalesces buffers of the same size within each group, which keeps fragmentation low even for models that mix full-attention, sliding-window, and recurrent layers. It also manages the host and disk tiers and the migration of pages between them, and it exposes a clean Python API for per-layer buffer configuration. These properties are what make compression on the cross-request path possible, described in its own section below. The compression manager binds to V2 once V2 is built, and the cold-page codec plugs into V2's page migration path, so V2 keeps ownership of pools, mappings, and migration while the codec decides how a page is stored off the GPU.
...
Second, repeated quantize-and-dequantize round trips do not compound. Quantization is a projection onto a finite set of values, not a fresh random error each time. For a 16-bit source we showed analytically that after the first round trip the encoded state can change at most once more. An FP8 source is rounded again when the restored value is written back to FP8, so we measured it. Figure 14 (left) puts the production encode and decode kernels through eight round trips on FP16, BF16 and FP8 inputs that enumerate every finite FP8 code and the exponent and subnormal edges of the 16-bit formats. The first restore changes 55 to 71% of the values, which is the expected rounding. The second round trip changes nothing in FP16 or BF16 and 18 of 9,216 FP8 values; the fourth and eighth change nothing in any format. Figure 14 (right) is the end-to-end check: Qwen3-8B with a BF16 KV cache on AIME24 and AIME25, 30 questions with 8 seeds each, with every page put through the round trips in a standalone harness on B200. End to end, one round trip moves accuracy by 2.08 and 1.67 points on the two sets, within the noise of this setup where most generations hit the 32K output cap. The second round trip reproduces all 480 outputs of the first exactly, so accuracy after two round trips is identical to accuracy after one.
...
<p align="center"><sub><em>Figure 14: Repeated NVFP4 round trips. Left: share of values changed by each round trip for FP16, BF16 and FP8 inputs on the native kernels; FP8 changes 18 values once more at the second round trip, then nothing. Right: AIME24 and AIME25 accuracy after 0, 1, and 2 round trips.</em></sub></p>
