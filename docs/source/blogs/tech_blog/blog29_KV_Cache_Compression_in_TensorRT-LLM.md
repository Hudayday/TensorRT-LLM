...
Second, repeated quantize-and-dequantize round trips do not compound. Quantization is a projection onto a finite set of values, not a fresh random error each time. For a 16-bit source we showed analytically that after the first round trip the encoded state can change at most once more. An FP8 source is rounded again when the restored value is written back to FP8, so we measured it. Figure 14 measures this as a distance. We take every finite FP8 code and every FP16 and BF16 value whose block scale is a normal E4M3 code, shuffle them into blocks of 16, and put each block through eight quantize-and-restore round trips of the codec. After each round trip we record, for every value, how far it is from its original value and how far it is from where it was after the previous round trip, both as a fraction of the block maximum. The left panel is the distance to the original: after the first round trip the median is 0.3 to 0.7% of the block maximum, the 90th percentile about 4%, and the worst value 17%, which is half an E2M1 step at the top of the range. That is the quantization error, and it is the same after two, four and eight round trips. The middle panel is the distance to the previous round trip: it equals the left panel at the first round trip and is exactly zero for every value from the second round trip on. The only exception we found is the exhaustive FP8 layout that lays all 254 codes out in order: there 18 of 1,024 values move once more at the second round trip, by at most 22% of the block maximum, and never again. These numbers come from a scalar reference model of the codec that reproduces the change counts of the production kernels exactly, including those 18 values. Figure 14 (right) is the end-to-end check: Qwen3-8B with a BF16 KV cache on AIME24 and AIME25, 30 questions with 8 seeds each, with every page put through the round trips in a standalone harness on B200. End to end, one round trip moves accuracy by 2.08 and 1.67 points on the two sets, within the noise of this setup where most generations hit the 32K output cap. The second round trip reproduces all 480 outputs of the first exactly, so accuracy after two round trips is identical to accuracy after one.
...
<p align="center"><sub><em>Figure 14: Repeated NVFP4 round trips. Left: distance of every value from its original after each round trip, as a percentage of the block maximum, for FP16, BF16 and FP8 inputs (dot median, bar to the 90th percentile, tick at the maximum). The distribution is the same after every round trip, so the error does not grow. Middle: the same distance measured against the previous round trip; it is zero for every value from the second round trip on. Right: AIME24 and AIME25 accuracy after 0, 1, and 2 round trips.</em></sub></p>
