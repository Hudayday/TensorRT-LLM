...
Second, repeated quantize-and-dequantize round trips do not compound. Quantization is a projection onto a finite set of values, not a fresh random error each time. For a 16-bit source we showed analytically that after the first round trip the encoded state can change at most once more. An FP8 source is rounded again when the restored value is written back to FP8, so we measured it. Figure 14 (left) puts the production encode and decode kernels through eight round trips on FP16, BF16 and FP8 inputs that enumerate every finite FP8 code and the exponent and subnormal edges of the 16-bit formats. The metric is a count, not an error size: after each round trip we compare every restored value with the value it had after the previous round trip and report the share that changed. The first round trip is ordinary quantization, so most values change, 55 to 71% on these inputs. If rounding drifted, later round trips would keep changing values. They do not: the second round trip changes nothing in FP16 or BF16 and 18 of 9,216 FP8 values, and the fourth and eighth change nothing in any format. Every value that leaves the GPU a second time comes back bit-identical to the first time. Figure 14 (right) is the end-to-end check: Qwen3-8B with a BF16 KV cache on AIME24 and AIME25, 30 questions with 8 seeds each, with every page put through the round trips in a standalone harness on B200. End to end, one round trip moves accuracy by 2.08 and 1.67 points on the two sets, within the noise of this setup where most generations hit the 32K output cap. The second round trip reproduces all 480 outputs of the first exactly, so accuracy after two round trips is identical to accuracy after one.
...
<p align="center"><sub><em>Figure 14: Repeated NVFP4 round trips. Left: after each round trip (quantize to NVFP4, restore to the source format), the share of values that differ from the previous round trip, for FP16, BF16 and FP8 inputs on the native kernels. The first round trip is ordinary quantization rounding; from the second on, 0 means every value is bit-identical, so the error does not accumulate. FP8 changes 18 of 9,216 values once more at the second round trip, then nothing. Right: AIME24 and AIME25 accuracy after 0, 1, and 2 round trips.</em></sub></p>
