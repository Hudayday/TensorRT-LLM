...
Second, repeated quantize-and-dequantize round trips do not compound. Quantization is a projection onto a finite set of values, not a fresh random error each time. For a 16-bit source we showed analytically that after the first round trip the encoded state can change at most once more, and on real KV values it does not change at all. An FP8 source is different: the restored value is rounded again when it is written back to FP8, so we measured that case directly. Figure 14 shows both. The left panel is Qwen3-8B with a BF16 KV cache on AIME24 and AIME25, 30 questions with 8 seeds each, using the NVFP4 quantizer from Transformer Engine in a standalone harness on B200 and applying the round trips to every page. The first round trip changes 98.5% of the 13 billion compared KV values, which is the expected rounding. The second round trip changes none of them, and neither do the fourth or the eighth. The middle panel runs the production encode and decode kernels on synthetic FP16, BF16 and FP8 inputs that cover every finite FP8 code and the exponent and subnormal edges of the 16-bit formats. The first restore changes 55 to 71% of the values; the FP8 set changes 18 of 9,216 values once more on the second round trip; nothing changes after that in any format. End to end, one round trip moves accuracy by 2.08 and 1.67 points on the two sets, within the noise of this setup where most generations hit the 32K output cap. The second round trip reproduces all 480 outputs of the first exactly, so accuracy after two round trips is identical to accuracy after one.
...
<p align="center"><sub><em>Figure 14: Repeated NVFP4 round trips. Left: real Qwen3-8B BF16 KV, share of values changed by one more round trip. Middle: the native kernels on synthetic FP16, BF16 and FP8 inputs; FP8 changes 18 values once more at the second round trip, then nothing. Right: AIME24 and AIME25 accuracy after 0, 1, and 2 round trips.</em></sub></p>
