...
As Large Language Models (LLMs) move into long-context and agentic deployments—multi-turn assistants, tool-calling loops, and extended reasoning—the amount of reusable Key-Value (KV) state that a serving system accumulates grows with every turn. A prefix computed once can be reused many times, but only if it is still present somewhere in the cache hierarchy when the next request arrives. When the GPU cannot hold enough of that state, the system evicts pages and later recomputes them, which creates redundant prefill work, raises time-to-first-token (TTFT), and caps the effective size of the KV cache a deployment can afford. Two resources are at stake:

- **Capacity**: how many KV pages the GPU, host, and disk tiers can retain, which bounds the cache hit rate a reuse-heavy workload can reach.
- **Data movement**: how many bytes cross the GPU–host and host–disk boundaries when pages are offloaded and brought back, which competes with the prefill and decode traffic the workload actually needs.

A wide range of KV cache compression methods have been proposed to address these bottlenecks. They can be broadly classified along two dimensions: **what** is compressed (which tokens are retained—eviction—versus how the retained values are stored—quantization or another compact encoding) and **when and where** the compression runs (inside the forward iteration, so that the attention kernel consumes the compressed state directly, versus at a storage boundary, when pages move between the GPU and a colder tier). TensorRT LLM already covers the first class through active KV cache quantization and the [sparse attention framework](blog17_Sparse_Attention_in_TensorRT-LLM.md); this blog addresses the second, which the serving lifecycle supports at natural, stable points—between forward steps and at hot/cold page transitions—without any change to the attention kernel.
