...
The clearest picture of what agentic serving asks from a KV cache comes from replaying real agent runs. [InferenceX](https://inferencex.semianalysis.com/) is SemiAnalysis's public inference benchmark; its AgentX lane records coding-agent sessions (model calls, tool calls, and the growing conversation between them) and replays them against a serving stack with synthetic content but the original token counts, prefix structure, and tool-call timing. The traces are published on the [InferenceX datasets page](https://inferencex.semianalysis.com/datasets), and the same replay methodology is described in our [trace-replay blog](blog27_Evaluating_Agentic_Serving_with_Trace_Replay_and_Job_Level_Metrics.md). The numbers from these traces are what motivate this work. A coding-agent job is not one request but a chain of them: the Coder traces average 38.7 LLM requests per job, and because every tool result is appended to one growing conversation, the prompt of each request is mostly text the system has already seen. The median input length per request is 14.4k tokens, the 90th percentile is 37.2k, and the tail reaches 210k, while each turn emits only about 110 output tokens. In a representative trace, about 34% of every prompt is the shared system prompt and about 63% is the job's own history, leaving roughly 3% fresh tokens. The optimal prefix-cache hit rate of these jobs is therefore 96.5% on average, and on the 256k-context AgentX trace used in our GB300 campaigns the trace ceiling is 95.5% to 98.0%, which a serving system reaches (93% to 98% measured) as long as the cache still holds the prefixes. The hit rate is the dominant term in agentic serving: when the KV-cache pool overflows, the trace-replay sweep shows the measured hit rate falling off an eviction cliff from 0.97 to 0.73 for Coder jobs, and job-level throughput degrades with it, because every lost prefix is prefilled again from scratch. Retaining evicted pages in host memory restores the hit rate to nearly the optimum. In other words, for agentic workloads the KV cache is not a performance accessory; losing it is paid for in redundant prefill on almost every turn.

Three things follow for the KV cache of an agentic deployment:

- **KV cache size, per request and across the workload.** Each request carries tens of thousands of tokens of KV, a job produces dozens of such requests, and many jobs run concurrently. The volume of KV that is worth keeping is much larger than any single request, and because consecutive turns share their prefix, the same KV pages are requested again and again across the workload rather than once.
- **Capacity, and the data movement it implies.** How many KV pages the GPU, host, and disk tiers can retain decides whether a prefix is still present at the next turn. Once the GPU pool overflows, pages must live in host memory or disk to stay reusable, and every page kept there costs bytes of capacity while every page brought back costs bytes across the GPU-host or host-disk boundary, on top of the prefill and decode traffic the workload actually needs. Capacity and movement are the same budget seen from two sides.
- **A method that applies to every model.** Tool-driven agent workloads are memory-heavy for every model that runs them, whether it uses conventional MHA/GQA attention, MLA latent pages, or a hybrid layout that mixes attention with recurrent state. A compression method that works for one attention layout or one runtime dtype helps one deployment; what is needed is a method that applies across models and cache structures without model-specific kernels or changes to the attention path.
