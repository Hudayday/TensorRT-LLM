...
def dummy_calibration_pt(tmp_path):
    ...
    calibration = {
        ...
        "R": torch.ones(2, 2, 4, dtype=torch.float32) * 0.5,
        "omega": torch.arange(4, dtype=torch.float32),
        "phi": torch.zeros(2, 2, 4, dtype=torch.float32),
    }
...
class TestIsBehaviorLayerMethod:
    ...
    def test_dsa_is_legacy(self):
        assert DeepSeekSparseAttentionConfig(
        ).is_behavior_layer_method is False

    def test_skip_softmax_is_legacy(self):
        assert SkipSoftmaxAttentionConfig(
        ).is_behavior_layer_method is False
...
class TestTriAttentionConfig:
    ...
    def test_field_overrides(self):
        cfg = TriAttentionConfig(top_B=256,
                                 beta=64,
...
class TestTriAttentionClass:
    ...
    def test_init_requires_calibration_path(self):
        ...
        with pytest.raises(ValueError, match="requires calibration_path"):
            TriAttention(kv_cache_manager=None,
                         top_B=32,
...
class TestFactory:
    ...
    def test_returns_none_for_rocket(self):
        assert create_sparse_attention_manager(
            RocketSparseAttentionConfig(), kv_cache_manager=None) is None

    def test_returns_none_for_dsa(self):
        assert create_sparse_attention_manager(
            DeepSeekSparseAttentionConfig(), kv_cache_manager=None) is None

    def test_returns_none_for_skip_softmax(self):
        assert create_sparse_attention_manager(
            SkipSoftmaxAttentionConfig(), kv_cache_manager=None) is None

    def test_raises_type_error_for_non_v2_cache_mgr(self,
                                                     dummy_calibration_pt):
        cfg = TriAttentionConfig(top_B=32,
                                 beta=16,
                                 calibration_path=dummy_calibration_pt)
        # Non-V2 instance (string here, but any non-V2 object would trigger)
        with pytest.raises(TypeError, match="KVCacheManagerV2"):
            create_sparse_attention_manager(cfg,
                                            kv_cache_manager="not_a_v2")

    @CUDA_REQUIRED
    def test_returns_triattention_instance_with_v2(self,
                                                   dummy_calibration_pt):
        # Stub a minimal V2-class instance: subclass KVCacheManagerV2 with
        # __init__ bypassed via __new__ to avoid real cache allocation.
