...
class TestBaseABC:
    ...
    def test_length_adjustment_marks_target_and_draft_v2(self):
        ...
        assert draft.generation_capacity_only is True
...
class TestFactory:
    ...
    def test_spec_validation_is_an_empty_hook_by_default(self):
        # The base accepts every speculative mode: methods that do not touch
        # the draft KV (e.g. offloading) work with any of them. Evicting
        # methods override validate_spec_draft_kv with their own rules.
        from tensorrt_llm._torch.speculative.interface import SpeculativeDecodingMode

        m = BaseKVCacheCompressionManager(
            _v2_manager(is_draft=False),
            spec_config=SimpleNamespace(spec_dec_mode=SpeculativeDecodingMode.DFLASH),
        )
        assert m.spec_config is not None
