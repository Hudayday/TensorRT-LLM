...
def _manager(*, is_draft: bool, generation_capacity_only: bool = False) -> KVCacheManagerV2:
...
def test_manager_initializes_capacity_only_policy_to_false() -> None:
    ...
    assert manager.generation_capacity_only is False
...
def test_capacity_only_is_scoped_to_target_manager() -> None:
    ...
    target = _manager(is_draft=False, generation_capacity_only=True)
...
def test_capacity_only_completion_preserves_history() -> None:
    manager = _manager(is_draft=False, generation_capacity_only=True)
...
def test_generation_update_has_no_request_compaction_marker() -> None:
    ...
    assert "py_kv_cache_generation_capacity_only" not in vars(request)
...
def test_v2_separates_logical_length_from_physical_capacity(
    manager_kwargs: dict,
    is_draft: bool,
