...
def test_native_cold_page_codec_ownership_is_restored_after_failure() -> None:
    if os.environ.get("TLLM_KV_CACHE_MANAGER_V2_BACKEND", "cpp").lower() != "cpp":
        pytest.skip("C++ backend only")
    ...
    manager = KVCacheManager(_make_config(), cold_page_codec=codec)
    try:
        assert manager.init_config.tokens_per_block == 4
    finally:
        manager.shutdown()
